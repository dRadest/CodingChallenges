...
int Solution::canCompleteCircuit(const vector<int> &A, const vector<int> &B) {
    int gas = 0, index = 0, sum = 0;
    
    // traverse gas stations
    for(int i = 0; i < A.size(); i++){
        // add difference in added gas and cost to sum and gas
        sum += (A[i] - B[i]);
        gas += (A[i] - B[i]);
        if(gas < 0){ // reset the starting gas station
            gas = 0;
            index = i+1;
        }
    }
    // overall sum of differences has to be above or equal to zero
    // to be able to make the round
    if(sum >= 0){
        return index%(A.size());
    }
